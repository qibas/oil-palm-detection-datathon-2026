...
raw number table. **Before quoting any number into prose, check `00_ANGKA_FINAL.md`** — the package holds
numbers from nine different configurations (full model vs photo variant, pooled vs within-census AP,
1/2/3/6/24 columns, two window sizes, four horizons) and several pairs are *not comparable*; that file
marks which are paper-primary and which must never be set side by side. It is generated by
`python make_final_table.py` from `00_RINGKASAN.csv` — regenerate it after adding rows, never hand-edit it. Prose documentation is in Indonesian; code docstrings are mixed (Layer 1 English,
...
- **Layer 1** (`layer1_build/`, `layer1_data_audit/`) — UAV RGB imagery from Roboflow `ds_B`. Crown
  ...
  a separate evidence line and is never merged with `ds_B` either. Its numbers live in
  `layer1_build/stage1_summary.json` and are reported in `00_HASIL.md` §2.6 — **one fold, one
  seed**, so `paired()` does not apply and they must never be set against ds_B's 3-fold
  mean ± std. Two caveats travel with them: the `full` control (crown + context) is *worse*
  than crown alone (ROC-AUC 0.880 vs 0.952), and the detector over-predicts by 36%
  (1.72 boxes/image vs 1.267 GT), so mAP50 0.95 does not mean the tree count is right.
...
v3 deliberately breaks two locked rules (`WINDOW=3`, and **`INTERFACE.md` prohibition #5**,
which requires genotype — note the package has *two* differently-numbered six-prohibition
lists, `README.md`'s and `INTERFACE.md`'s; always name which). That is
...
the observed value under the strictest stratum. `INTERFACE.md` prohibition #5 stays fully binding for `run_real.py`
