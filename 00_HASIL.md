...
|---|---|---|---|
| Deteksi tajuk | **F1 pusat** | **0,960 ± 0,024** | **metrik utama** — pusat tajuk pada pohon unik; YOLOv12n, **3 lipatan × 30 epoch**. ⚠ ketiganya **bukan** satu lingkungan — lihat baris "Lingkungan pelatihan" di Bagian 4 |
...
| ~~Deteksi YOLO 1 lipatan, 15 epoch~~ | **DITUTUP sebagian** | Kini YOLOv12n, **3 lipatan × 30 epoch**, dengan mean ± std. ⚠ **Koreksi:** baris ini dulu menulis "dalam satu lingkungan". Itu **tidak benar** — lihat baris "Lingkungan pelatihan" di bawah. Yang tersisa dan tetap harus dinyatakan: mAP berlangit-langit label, dan **`52000_20000` menyeret seluruh std** — turun di semua metrik (mAP50 0,605 lawan 0,726/0,731) karena anotasinya tidak konsisten (CV kotak 0,327 lawan 0,126/0,170), **bukan** karena efek situs. Pada metrik pusat jaraknya menyusut 3× (0,933 lawan 0,969/0,977), persis karena pusat adalah bagian label yang dapat dipercaya |
| **Lingkungan pelatihan Lapisan 1** | **3 lipatan, ≥2 lingkungan** | Ketiga lipatan **tidak** dilatih di satu lingkungan, dan buktinya ada di berkas larinya sendiri. `fold0` dan `fold2` berbagi satu mesin (path `args.yaml` menunjuk pengguna lain) dan `args.yaml`-nya memuat empat kunci (`channels_last`, `dgrad`, `dlam`, `dlog`) yang **tidak ada** di ultralytics 8.4.96 — yakni build yang lebih baru. `fold1` **tidak punya `args.yaml` sama sekali**, log epoch-nya mulai dari **2** bukan 1, dan waktu larinya 53,5 mnt lawan 44,9 mnt. Akibatnya **±std memuat ragam lingkungan, bukan hanya ragam antar-situs**, dan itu berlaku untuk `0,960 ± 0,024` maupun `5,54 ± 0,12`. Besarnya kini **terukur**, bukan ditaksir: `run_1fold_yolov12n.ipynb` melatih ulang `fold0` dengan seed dan setelan yang identik, hanya mesin dan versi ultralytics yang berbeda → presisi pusat bergeser **+0,0209 (1,1 std)**, RMSE **−0,0145 (1,35 std)**, F1 **+0,0111 (0,47 std)**, derajat dalam **−0,0486 (0,41 std)**, mAP50 **−0,0240 (0,34 std)**. Pergeseran akibat lingkungan saja karena itu **sebesar, bahkan melebihi**, pita yang dilaporkan sebagai ketidakpastian antar-situs. Angka mentah: `layer1_build/yolo12_results/centre_eval_1fold.json`. Penutupan yang benar adalah `FOLDS=all python train_folds_gpu.py` dalam satu lingkungan (~67 mnt); sampai itu dilakukan, pita ±std **tidak boleh** dibaca sebagai murni antar-situs |
